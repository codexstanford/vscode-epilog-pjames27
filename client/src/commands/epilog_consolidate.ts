...
const validExtensions = new Set([ 
    LANGUAGE_ID_TO_FILE_EXTENSION.get(EPILOG_DATASET_LANGUAGE_ID), 
...
function generateFilenameUnusedInList(extension: string, existingFilenames: string[], prefix: string = 'newfile') {
...
function getDocumentDir(document: vscode.TextDocument) {
...
function parseEpilogBuildFile(document: vscode.TextDocument) : [boolean, boolean, string, [string, string][], boolean] {
    ...
    let filenamesToBuildAndNewFilenames: [string, string][] = [];
...
function validateEpilogBuildFilenamesAndConvertToAbsolute(
    document: vscode.TextDocument, 
    filenamesToBuildAndNewFilenames: [string, string][], 
    prefixValue: string) : [[string, string][], boolean] 
    {
    ...
    let absFilenamesToBuild: Set<string> = new Set();
    let absNewFilenames: Set<string> = new Set();
    let absFilenamesToBuildAndNewFilenames: [string, string][] = [];
...
function generateNeededNewFilenames(document: vscode.TextDocument, absFilenamesToBuildAndNewFilenames: [string, string][], relFilenamesToBuildAndNewFilenames: [string, string][], prefixValue: string) : [[string, string][], boolean] {
...
async function consolidate_EpilogBuild() {
    ...
    if (document.languageId !== EPILOG_BUILD_LANGUAGE_ID) {
    ...
    for (const [absFilenameToBuild, absNewFilename] of absFilenamesToBuildAndNewFilenames) {
        ...
        const choice = await vscode.window.showWarningMessage(
            `File "${absNewFilename}" already exists. Do you want to overwrite it?`,
            'Yes',
            'No'
        );
        if (choice === 'Yes') {
